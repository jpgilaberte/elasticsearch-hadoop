...
`es.mapping.definition` (default none)::
The custom mapping index definition. Only apply if index does not exist.

For example:
[source,ini]
----
# defining shards and replicas
es.mapping.definition = '{
                           "settings": {
                             "number_of_shards": 3,
